...
//								$sql_consul_entradas=$sql_consul_entradas." and MONTH(tlob.Fecha)=3 and year(tlob.Fecha)=2013 and DAY(tlob.Fecha)=31 ";
...
$error2=almacena_entradas_salidas(0,"TMEntradas","no",$mes);

echo "<br><br> error2: $error2 <br>";
if($error2=="no") //si no se presentaron errores al insertar las salidas
	$error2=almacena_entradas_salidas(1,"TMSalidas","no",$mes);
...
///PARA QUITAR
echo  " and MONTH( tlob.Fecha) = month(getdate())-1 and year(tlob.Fecha)=year(getdate()) and day( tlob.Fecha)=day(dateadd(d,-1,dateadd(m,1,convert(datetime, '".date("Y")."' + '".$mes."' + '01'))))";

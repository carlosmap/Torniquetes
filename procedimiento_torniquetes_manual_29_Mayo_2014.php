				...
				$informacion_departamentos[$f][$c]=$datos_dep["nombre"];
					...
					$sql_consul_entradas="
							...
							WHERE
								...
								$sql_consul_entradas=$sql_consul_entradas." and MONTH( tlob.Fecha)='".$messs."' and year(tlob.Fecha)='".$annno."' and day(tlob.Fecha)='".$diiias."' ";  //TEMPORAL
/*					
...
echo "Inicia entradas ";
echo ( "<br>HORA DE inicio $tabla ".$hora["hours"] . ":" . $hora["minutes"] . ":" . $hora["seconds"]." error: $error <br><br> " ); 
//ejecuta la funcion, para las entradas
list($error2,$informacion_departamentos_entradas,$info_divisiones_entradas,$sql_error,$sql_error2)=almacena_entradas_salidas(0,"TMEntradas","no",5);

echo "Inicia salidas ";
echo ( "<br>HORA DE inicio $tabla ".$hora["hours"] . ":" . $hora["minutes"] . ":" . $hora["seconds"]." error: $error" ); 

if($error2=="no") //si no se presentaron errores al insertar las salidas
	list ($error2,$informacion_departamentos_salidas,$info_divisiones_salidas,$sql_error,$sql_error2)=almacena_entradas_salidas(1,"TMSalidas","no",5);
...
//		echo "--------------- SQL ERROR <BR>".$sql_error2."<br><br>";
	...
	if(trim($cur_t)=="")
	{
		...
		$sql_err=" delete from HojaDeTiempo.dbo.TMEntradas where YEAR(Fecha_registro)='".$annno."' and MONTH(Fecha_registro)='".$messs."' and DAY(Fecha_registro)='".$diiias."' ";
		$cur_err=mssql_query($sql_err);
		
		if(trim($cur_err)!="")
		{
			$sql_err=" delete from HojaDeTiempo.dbo.TMSalidas where YEAR(Fecha_registro)='".$annno."' and MONTH(Fecha_registro)='".$messs."' and DAY(Fecha_registro)='".$diiias."' ";
			$cur_err=mssql_query($sql_err);
		}
...
//echo "<br>".(count($informacion_departamentos_entradas, COUNT_RECURSIVE)-count($informacion_departamentos_entradas))." --------------- <br>";
		...
		$pTema=$pTema." </table><table  width='100%' border='1' >";
		$pTema=$pTema."<tr>
...
//				echo $informacion_departamentos_entradas[$i][0]."<br>";
				 ...
				 $pTema=$pTema.'<tr class="Estilo2" align="center">
					...
					<td>'.$informacion_departamentos_entradas[$i][3].' --'.$i.' ** '.(count($informacion_departamentos_entradas, COUNT_RECURSIVE)-count($informacion_departamentos_entradas)).' ** '.count($informacion_departamentos_entradas, COUNT_RECURSIVE).' -- </td>
				  </tr>';
...
//echo "<br>".(count($informacion_departamentos_salidas, COUNT_RECURSIVE)-count($informacion_departamentos_salidas))." --------------- <br>";
		$pTema=$pTema."<tr><td colspan=4 >&nbsp;</td></tr><tr><td colspan=4  class='Estilo2' align='center'>Informacion de las salidas <td></tr>";
		//envia el correo con la  informacion del procedimiento
		$total=0;
		for($i=0;$i<=(count($informacion_departamentos_salidas, COUNT_RECURSIVE)-count($informacion_departamentos_salidas)) ;$i++)
		{
//			for($c=0;$c<=4;$c++)
			{
//				echo $informacion_departamentos_salidas[$i][0]."<br>";
				 ...
				 $pTema=$pTema.'<tr class="Estilo2" align="center">
					...
					<td>'.$informacion_departamentos_salidas[$i][3].'  -- '.$i.' **** '.(count($informacion_departamentos_salidas, COUNT_RECURSIVE)-count($informacion_departamentos_salidas)).' ** '.count($informacion_departamentos_salidas, COUNT_RECURSIVE).' -- </td>
				  </tr>';
				$total+=$informacion_departamentos_entradas[$i][1];
			}
		}

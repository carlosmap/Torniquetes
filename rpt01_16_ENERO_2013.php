...
if($recarga==2)
{
	...
	$sql_consul_perso="  select distinct(t.unidad),NoTarjeta,u.retirado, u.nombre as Nombres,u.apellidos as Apellidos, d.nombre as Departamento from HojaDeTiempo.dbo.TMEntradas as t
...
//echo $dias[$z]."    indice ".$z."<br>";
			...
			//*********
						...
						{
							//almacena la infromacion de la hora de entrada
							if($datos_entrada=mssql_fetch_array($cur_entradas))
							{
								$cont_entra++;
								$hora_entrada=$datos_entrada["Hora"];
							}
							//almacena la infromacion de la hora de salida
							if($datos_salidas=mssql_fetch_array($cur_salidas))
							{
								$cont_salidas++;
								$hora_salida=$datos_salidas["Hora"];
							}
							//realiza el calculo de la cantidad de horas entre las entradas y salidas, pasandolo a minutos
